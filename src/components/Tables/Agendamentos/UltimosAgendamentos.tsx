...
async function fetchLastAgendamentos(codigoUsuario?: number) {
    ...
    const response = await baseApi.get<Agendamentos[]>(`/agendamentos/ultimos/${codigoUsuario}`);
...
export function UltimosAgendamentos() {
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <h1 className="text-xl text-primary-logo font-semibold">Últimos Agendamentos</h1>
            {/* {data && ( */}
            <DataTable
                data={data ? data : []}
                columns={columnsHome}
